...
const { graphConfig, getGraphSize, getInnerWidth } = require('./config')
...
const Radar = function (size, radar) {
  ...
  function plotRingNames(quadrantGroup, rings, quadrant) {
    rings.forEach(function (ring, i) {
      const ringNameWithEllipsis = ring.name().length > 25 ? ring.name().slice(0, 25) + '...' : ring.name()
  ...
  function selectQuadrant(order, startAngle) {
    ...
    var moveRight = ((1 + adjustX) * (0.8 * getInnerWidth() - size)) / 2
    var moveLeft = ((1 - adjustX) * (0.8 * getInnerWidth() - size)) / 2
